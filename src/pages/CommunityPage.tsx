...
import {
  Search, X, Users, ArrowLeft, Lock, Shield,
  Settings, LogOut, Trash2, ChevronDown,
} from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="text-lg font-bold text-text-primary dark:text-dark-text text-center mb-1">
          Delete community
        </h2>
        <p className="text-sm text-text-secondary dark:text-gray-400 text-center mb-4">
          This will permanently delete <strong>{communityName}</strong> and all its posts and
          replies. This action cannot be undone.
        </p>
        <p className="text-sm text-text-primary dark:text-dark-text mb-2">
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} className="btn btn-secondary flex-1" disabled={deleting}>
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={confirmText !== communityName || deleting}
    ...
    <aside className="hidden lg:flex flex-col gap-4 w-72 shrink-0">
      <div className="card p-5 flex flex-col gap-4">
        ...
        {community.description && (
          <p className="text-sm text-text-secondary dark:text-gray-400 leading-relaxed text-center">
            {community.description}
          </p>
        )}
        ...
        {isMember ? (
          <div className="flex flex-col gap-2">
            {isOwner && (
              <div className="badge-primary text-xs justify-center py-1.5">
                <Shield className="w-3 h-3" /> You own this community
              </div>
            )}
            {!isOwner && (
              <button
            ...
            {isOwner && (
              <button
                ...
                className="btn btn-sm w-full gap-1.5 text-error hover:bg-red-50 dark:hover:bg-red-900/20 bg-transparent border border-error/30"
              >
      ...
      <div className="card p-4">
        <h3 className="text-sm font-semibold text-text-primary dark:text-dark-text mb-2">
          Community rules
        </h3>
        <ol className="list-decimal list-inside space-y-1">
          {[
            'Be respectful to everyone',
            'Stay on topic for this subject',
...
export default function CommunityPage() {
  ...
  // ── Fetch community + membership + initial posts ───────────────────────────
  useEffect(() => {
    ...
    const init = async () => {
      setLoading(true)
      try {
        const [{ data: comm, error: commErr }, { data: posts, error: postsErr }] =
          await Promise.all([
        ...
        setCommunity(comm)
        setPosts(posts ?? [])
        setHasMore((posts?.length ?? 0) === PAGE_SIZE)
        setPage(0)
    ...
    searchDebounce.current = setTimeout(async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select('*')
          .eq('community_id', communityId)
          .textSearch('', search.trim(), { config: 'english' })
        ...
        <div className="flex gap-6">
          ...
          <div className="hidden lg:block w-72 shrink-0">
            <div className="card p-5 space-y-3">
              ...
              <div className="skeleton h-3 w-1/3 rounded mx-auto" />
    ...
    <>
      <div className="page-container-wide">
        ...
        <div className="flex items-center gap-2 mb-5 text-sm text-text-secondary dark:text-gray-400">
          ...
          <span>/</span>
          <span className="text-text-primary dark:text-dark-text font-medium truncate max-w-xs">
            {community.name}
          </span>
        </div>
        ...
        <div className="lg:hidden card p-4 flex items-center gap-3 mb-5">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-text-primary dark:text-dark-text truncate flex items-center gap-1">
              {community.name}
              {community.is_private && <Lock className="w-3.5 h-3.5 text-text-secondary shrink-0" />}
            </h1>
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
              ...
              <div className="search-wrapper flex-1">
                ...
                {searching && (
                  <span className="absolute right-3">
                    <span className="spinner w-3.5 h-3.5 text-text-secondary" />
                  </span>
                )}
                {search && !searching && (
                  <button
            ...
            {search && !searching && (
              <p className="text-xs text-text-secondary dark:text-gray-400">
                {displayPosts.length} result{displayPosts.length !== 1 ? 's' : ''} for "{search}"
              </p>
            )}
                ...
                {search ? (
                  ...
                  <>
                    ...
                    {isMember
                      ...
                      : <p className="text-sm">Join the community to post.</p>
                    }
                  </>
                )}
              </div>
